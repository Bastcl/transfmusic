...
async function searchYouTubeTrack(trackName, artist) {
    try {
        const response = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&q=${encodeURIComponent(`${trackName} ${artist}`)}&type=video&key=TU_YOUTUBE_API_KEY`);
        const data = await response.json();
